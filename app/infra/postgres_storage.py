...
class PostgresStorage:
    ...
    def update_display_name(self, *, user_id: str, display_name: str) -> UserProfileSummary:
        ...
        with self._pool.connection() as conn, conn.cursor() as cur:
            cur.execute(
                "UPDATE user_profiles SET display_name = %s, updated_at = %s WHERE user_id = %s::uuid",
            ...
            cur.execute(
                ...
                SELECT user_id::text AS user_id, email, display_name, role, status,
                       created_at::text AS created_at, updated_at::text AS updated_at, last_login_at::text AS last_login_at
                FROM user_profiles WHERE user_id = %s::uuid
                """,
                (clean_user_id,),
            )
            row = cur.fetchone()
    ...
    def like_deck(self, *, user_id: str, deck_id: str) -> None:
        ...
        with self._pool.connection() as conn, conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO deck_likes (user_id, deck_id, created_at)
    ...
    def unlike_deck(self, *, user_id: str, deck_id: str) -> None:
        with self._pool.connection() as conn, conn.cursor() as cur:
            cur.execute(
                "DELETE FROM deck_likes WHERE user_id = %s::uuid AND deck_id = %s::uuid",
        ...
        *,
        ...
        rows = self._deck_rows(
            ...
            SELECT d.id::text AS id, d.user_id::text AS user_id, d.name, d.source, d.format, d.bucket, d.visibility,
                   ...
                   MAX(CASE WHEN lk.user_id = %s::uuid THEN 1 ELSE 0 END) AS liked_by_me
            FROM user_decks d
            JOIN user_profiles p ON p.user_id = d.user_id
